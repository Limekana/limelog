...
// LimeLog under the old text; someone installing now meets the new policy at
...
const KEY = 'limelog-policy-seen';

/** Called from main.tsx before onboarding can run: a device that has never
...
export function notePolicyBaseline(): void {
  try {
    ...
    /* no storage: the note simply shows, which errs on the side of telling */
  }
}
