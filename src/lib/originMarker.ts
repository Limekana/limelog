...
//
// Note the suite wrinkle: LimeLog can inherit a session from NCC over the SSO
// bridge, so `app` records which app happened to observe the user first, not
// which one they signed up in. Platform and origin are identical across suite
// apps on one device, so the answer ACT-5 actually needs is unaffected.
...
export function scheduleOriginStamp(user: User | null): void {
  ...
  setTimeout(() => {
    void supabase.auth
      ...
      .catch((e) => console.warn('[limelog] origin marker stamp failed:', e));
  }, 0);
}
