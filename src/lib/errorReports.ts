...
// src/lib/errorReports.ts. What the privacy policy (NCC#50) promises, and
// this file therefore does:
...
const APP = 'limelog';
const KEY = 'limelog-error-reports'; // 'on' enables; anything else (or nothing) is off
...
export function buildReport(error: unknown, screen: string = currentScreen()): ErrorReport {
  ...
  return {
    ...
    platform: Capacitor.getPlatform(), // LimeLog ships no desktop build
