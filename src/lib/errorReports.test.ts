...
// Ported from NCC; the privacy policy (NCC#50) describes exactly this.
...
function thrown(message: string, name = 'TypeError'): Error {
  ...
  e.name = name;
  e.stack = `${name}: ${message}\n    at renderSet (https://localhost/assets/index-abc123.js?v=9:1:204511)\n    at x (https://localhost/assets/vendor.js:2:10)`;
  return e;
}
...
describe('what a report contains', () => {
  ...
  it('keeps stack frames but drops the stack\'s own copy of the message', () => {
    ...
    expect(r.stack).toContain('at renderSet');
    expect(r.message).toBe('balance for [email]');
  });

  it('fingerprints by error name and first frame, without the bundle query string', () => {
    expect(buildReport(thrown('a')).fingerprint).toBe('TypeError@at renderSet (https://localhost/assets/index-abc123.js:1:204511)');
...
describe('uncaught errors and rejections', () => {
  function fakeWindow() {
    ...
    return {
      addEventListener: (t: string, h: (ev: unknown) => void) => void (handlers[t] ??= []).push(h),
      removeEventListener: (t: string, h: (ev: unknown) => void) => void (handlers[t] = (handlers[t] ?? []).filter((x) => x !== h)),
      fire: (t: string, ev: unknown) => (handlers[t] ?? []).forEach((h) => h(ev)),
    };
  }
...
describe('the export includes what exists only on the server', () => {
  it('reads the user\'s feedback and error reports', async () => {
    ...
    expect((server.feedback as Row[]).map((r) => r.id)).toEqual(['f1']);
    expect((server.client_errors as Row[]).map((r) => r.id)).toEqual(['e1']);
  });
